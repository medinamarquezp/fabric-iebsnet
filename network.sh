...
function createOrgs() {
  ...
  while :
    do
      if [ ! -f "organizations/fabric-ca/madrid/tls-cert.pem" ]; then
        sleep 1
      else
        break
      fi
    ...
    infoln "Creating madrid Identities"

    createmadrid

    infoln "Creating bogota Identities"
  ...
  infoln "Generating CCP files for madrid and bogota"
...
# file for creating the network is "docker-compose-iebs-net.yaml" in the ``docker``
...
# call the script to create the channel, join the peers of madrid and bogota,
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    docker run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/madrid/msp organizations/fabric-ca/madrid/tls-cert.pem organizations/fabric-ca/madrid/ca-cert.pem organizations/fabric-ca/madrid/IssuerPublicKey organizations/fabric-ca/madrid/IssuerRevocationPublicKey organizations/fabric-ca/madrid/fabric-ca-server.db'
    docker run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/bogota/msp organizations/fabric-ca/bogota/tls-cert.pem organizations/fabric-ca/bogota/ca-cert.pem organizations/fabric-ca/bogota/IssuerPublicKey organizations/fabric-ca/bogota/IssuerRevocationPublicKey organizations/fabric-ca/bogota/fabric-ca-server.db'
...
CLI_DELAY=3
# channel name defaults to "universidades"
CHANNEL_NAME="universidades"
...
COMPOSE_FILE_BASE=docker/docker-compose-iebs-net.yaml
