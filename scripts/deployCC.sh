...
approveForMyOrg() {
  ...
  setGlobals $ORG
  set -x
  peer lifecycle chaincode approveformyorg -o localhost:7050 --ordererTLSHostnameOverride orderer --tls --cafile "$ORDERER_CA" --channelID $CHANNEL_NAME --name ${CC_NAME} --version ${CC_VERSION} --package-id ${PACKAGE_ID} --sequence ${CC_SEQUENCE} ${INIT_REQUIRED} ${CC_END_POLICY} ${CC_COLL_CONFIG} >&log.txt
  res=$?
  { set +x; } 2>/dev/null
  cat log.txt
...
commitChaincodeDefinition() {
  ...
  set -x
  peer lifecycle chaincode commit -o localhost:7050 --ordererTLSHostnameOverride orderer --tls --cafile "$ORDERER_CA" --channelID $CHANNEL_NAME --name ${CC_NAME} "${PEER_CONN_PARMS[@]}" --version ${CC_VERSION} --sequence ${CC_SEQUENCE} ${INIT_REQUIRED} ${CC_END_POLICY} ${CC_COLL_CONFIG} >&log.txt
  res=$?
  { set +x; } 2>/dev/null
  cat log.txt
...
chaincodeInvokeInit() {
  ...
  peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer --tls --cafile "$ORDERER_CA" -C $CHANNEL_NAME -n ${CC_NAME} "${PEER_CONN_PARMS[@]}" --isInit -c ${fcn_call} >&log.txt
  res=$?
  { set +x; } 2>/dev/null
  cat log.txt
...
chaincodeQuery() {
  ORG=$1
  setGlobals $ORG
  infoln "Querying on ${ORG} on channel '$CHANNEL_NAME'..."
  local rc=1
  local COUNTER=1
  # continue to poll
  # we either get a successful response, or reach MAX RETRY
  while [ $rc -ne 0 -a $COUNTER -lt $MAX_RETRY ]; do
    sleep $DELAY
    infoln "Attempting to Query ${ORG}, Retry after $DELAY seconds."
    set -x
  ...
  cat log.txt
  if test $rc -eq 0; then
    successln "Query successful on ${ORG} on channel '$CHANNEL_NAME'"
  else
    fatalln "After $MAX_RETRY attempts, Query result on ${ORG} is INVALID!"
  fi
}
...
## Install chaincode on peer0.madrid and peer0.bogota
infoln "Installing chaincode on peer0.madrid..."
installChaincode 1
infoln "Install chaincode on peer0.bogota..."
installChaincode 2

## query whether the chaincode is installed
queryInstalled 1

## approve the definition for madrid
approveForMyOrg 1

## check whether the chaincode definition is ready to be committed
## expect madrid to have approved and bogota not to
checkCommitReadiness 1 "\"madridMSP\": true" "\"bogotaMSP\": false"
checkCommitReadiness 2 "\"madridMSP\": true" "\"bogotaMSP\": false"

## now approve also for bogota
approveForMyOrg 2

## check whether the chaincode definition is ready to be committed
## expect them both to have approved
checkCommitReadiness 1 "\"madridMSP\": true" "\"bogotaMSP\": true"
checkCommitReadiness 2 "\"madridMSP\": true" "\"bogotaMSP\": true"
