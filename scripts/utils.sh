...
function printHelp() {
  ...
  if [ "$USAGE" == "up" ]; then
    ...
    println "    -c <channel name> - Name of channel to create (defaults to \"universidades\")"
    ...
    println
    println " Examples:"
    println "   network.sh up createChannel -ca -c universidades -s couchdb -i 2.0.0"
    ...
    println "    -ccep <policy>  - (Optional) Chaincode endorsement policy using signature policy syntax. The default policy requires an endorsement from madrid and bogota"
